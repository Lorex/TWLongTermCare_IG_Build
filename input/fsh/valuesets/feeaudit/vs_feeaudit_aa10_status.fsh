ValueSet: VS_TW_LTC_FeeAuditAA10Status
Id: vs-tw-ltc-feeaudit-aa10-status
Title: "支付審查－AA10 申報狀態"
Description: "此 ValueSet 用於表示支付審核系統服務記錄申報之 AA10（夜間緊急服務）申報狀態（aa10_status）。"
* ^identifier[0].system = "urn:ietf:rfc:3986"
* ^identifier[0].value = "urn:oid:2.25.3771695048331836787133083509745613180"

* ^url = "http://ltc-ig.fhir.tw/ValueSet/vs-tw-ltc-feeaudit-aa10-status"
* ^version = "0.1.0"
* ^status = #active
* ^experimental = false
* include codes from system CS_TW_LTC_FeeAuditAA10Status
