...
本頁說明長照服務提供單位及系統商如何使用 FHIR 傳送申報資料、查詢審核結果，以及辦理刪除或撤回。內容依據《衛生福利部 支付審核系統 API 規格說明書（照管平台）v2.2.1》說明文件，並繼承自 FHIR R4.0.1 及 TW Core IG 規範。

初次介接可先看下方的作業流程與填寫說明。要將既有 API 資料轉成 FHIR，可查閱後面的「原 API 欄位對照」，完整欄位、必填條件與範例詳見各 Profile 頁面。
...
3. **送出申報確認通知（`appCompletionNotice`）**：資料確認無誤後，依案件通知縣市承辦人員收件審查。**送出後，案件進入審查階段，服務紀錄鎖定。**
...
| 操作 | 處理範圍 | 填寫方式與限制 |
|---|---|---|
| 撤回服務紀錄（`appCancel`） | 撤回本月由該來源系統申報的服務紀錄 | 未填縣市代碼與核銷案號時，撤回整月資料。有填時，只撤回指定核銷案號。撤回適用於承辦人收件處理前的案件。|
...
一次申報使用一份 [LTCBundleFeeApply](StructureDefinition-LTCBundleFeeApply.html)，`Bundle.type` 填入 `collection`。每筆服務紀錄各建立一筆 [LTCClaimFeeApply](StructureDefinition-LTCClaimFeeApply.html)，放入 Bundle，每筆 Claim 只能有一筆 `item`。
...
服務內容、交通接送資料等補充資訊填在 `Claim.supportingInfo`，依資料類型選擇欄位，以下是範例：
...
照顧組合代碼請填在 `item.productOrService.coding`。目前 Profile 的條件檢核依這裡的代碼判斷，若只填 `text` 則不會觸發檢核。
...
`input` 與 `output` 的 Slice 以 `type.text` 區分。例如縣市代碼的 `type.text` 固定填入「縣市代碼」。
...
審核結果中的服務紀錄識別資料填在 [recordRef Extension](StructureDefinition-Ext-TW-LTC-FeeAudit-RecordRef.html)。比對原申報紀錄時，請讀取這裡的識別碼、來源系統與交易序號。
...
目前 Profile 要求 `ClaimResponse.patient` 填入一個 Patient Reference。核銷案號含多位個案時，依 Profile 說明填入代表個案或分案受理的個案。
...
API 回覆結果代碼、API 執行狀況及縣市代碼這三組術語用於原系統資料對照。API 執行狀況轉成 `Task.status` 的標準代碼，縣市代碼則填入 Organization 的識別碼或 `Task.input[cityCd]` 的字串值。

縣市主管機關的 `Organization.identifier.system` 使用 `http://ltc-ig.fhir.tw/identifier/feeaudit/city-cd`。此識別碼表示辦理支付審查的縣市主管機關；TW Core 郵遞區號用於地址。

核定金額類別使用本 IG 的代碼，相關欄位綁定強度為 `extensible`。與其他系統交換時，需確認所用代碼的定義與值集要求。

### 傳輸設定

本頁規範申報與審核資料的 FHIR 表達方式。原 API 的傳輸內容與回傳內容不另建 FHIR 資源，其欄位如下：

| 分類 | 原 API 欄位 |
|---|---|
| 呼叫 | `unitNo`、`requestDt`、`sourceSystem`、`apdata`、`checksum` |
| 回傳 | `rtncode`、`responseDt`、`errmsg`、`result`、`checksum`、`rtnSeq` |
{: .grid .rwd-table}

回傳的 `rtncode` 可查閱 [API 回覆結果代碼](ValueSet-vs-tw-ltc-feeaudit-rtncode.html)。
