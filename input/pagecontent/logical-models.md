...
[居家護理個案基本資料邏輯模型](StructureDefinition-HNBaseDataModel.html)／[居家護理主題與模型清單](home-nursing.html)
...
**Mapping 目標**: [欄位對照表](home-nursing-mapping.html)，對應個案、收案療程、問卷回覆、照護計畫、目標、觀察紀錄與處理任務等資源。

**使用場景**: 適用於居家護理機構整合收案、評估、照護執行及追蹤資料，作為來源表單與 FHIR 資源轉換的依據。

##### 在宅急症照護資料集邏輯模型 (HAHCareDataset)

[在宅急症照護資料集邏輯模型](StructureDefinition-HAHCareDataset.html)
...
**Mapping 目標**: [欄位對照表](hah-mapping.html)，對應 Patient、EpisodeOfCare、Encounter、CarePlan、Observation、MedicationAdministration、Composition 等資源。

**使用場景**: 適用於在宅急症照護團隊交換同一次療程的臨床資料，以及結案或轉院時提供完整照護摘要。流程與 Profiles 詳見[在宅急症主題](hah.html)。
...
基數表示欄位的出現次數，例如 `1..1` 為必填一筆、`0..*` 為選填且可重複。`MS` 表示實作需支援的欄位。對應的 FHIR Profile 定義交換時的資料型態、基數、值集與 invariant。
