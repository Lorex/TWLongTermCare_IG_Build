# Fee Audit - 臺灣長期照顧實作指引(TW LTC IG) v1.1.0
...
1. **刪除誤報資料（`ObjDel`）**：列出要刪除的服務紀錄識別碼。刪除成功或失敗的結果由查詢 A 取得。
1. **送出申報確認通知（`appCompletionNotice`）**：資料確認無誤後，依案件通知縣市承辦人員收件審查。**送出後，該案件不能再申報或刪除服務紀錄。**
...
一次申報使用一份 [LTCBundleFeeApply](StructureDefinition-LTCBundleFeeApply.md)，`Bundle.type` 填入 `collection`。每筆服務紀錄各建立一筆 [LTCClaimFeeApply](StructureDefinition-LTCClaimFeeApply.md)，放入 Bundle；每筆 Claim 只有一筆 `item`。

Bundle 也要放入相關的[個案 Patient](StructureDefinition-LTCPatient.md) 與[申報單位 Organization](StructureDefinition-Organization-twltc.md)，並依服務內容加入[照顧服務員 Practitioner](StructureDefinition-LTCPractitioner.md) 和[交通接送地點 Location](StructureDefinition-LTCLocationFeeAuditPlace.md)。這些都是放在 `Bundle.entry.resource` 的完整資源，Claim 再透過 Reference 參照它們。
...
刪除、申報確認、撤回及取消結果回報都使用 [LTCTaskFeeAudit](StructureDefinition-LTCTaskFeeAudit.md)。`Task.code` 填入操作代碼，`input` 填入操作所需資料，`output` 記錄處理結果。同次交易的任務使用相同的 `Task.groupIdentifier`。
...
查詢 A 的回覆使用 [LTCBundleFeeAuditStatus](StructureDefinition-LTCBundleFeeAuditStatus.md)，內含 Task；分案異常或刪除失敗的訊息另以 [LTCOperationOutcomeFeeAudit](StructureDefinition-LTCOperationOutcomeFeeAudit.md) 放入 Bundle，並由 Task 的 `output` 參照。
...
查詢 B 的回覆使用 [LTCBundleFeeAuditResponse](StructureDefinition-LTCBundleFeeAuditResponse.md)，其中一份 [LTCClaimResponseFeeAudit](StructureDefinition-LTCClaimResponseFeeAudit.md) 記錄一個核銷案號的審核結果。該案可能包含多筆申報 Claim。
...
審核結果中的服務紀錄識別資料填在 [recordRef Extension](StructureDefinition-Ext-TW-LTC-FeeAudit-RecordRef.md)。比對原申報紀錄時，請讀取這裡的識別碼、來源系統與交易序號；`itemSequence` 不能用來存放字串識別碼，也不足以識別跨 Claim 的服務紀錄。

統計筆數、個案數、暫付申請狀態、已處理交易序號及承辦人員填在 [auditSummary Extension](StructureDefinition-Ext-TW-LTC-FeeAudit-AuditSummary.md)。`requestor` 表示提出申報的服務提供方，審核承辦人員應填在 `auditSummary`。

總表與清冊的下載網址填在 [docUrl Extension](StructureDefinition-Ext-TW-LTC-FeeAudit-DocUrl.md)，每份文件各填一筆，包含文件類別與網址。
...
| FeeApply／case_svc_records | 照顧組合代碼`gov_item_cd` | `Claim.item.productOrService`，在`coding`填入[臺灣長照服務項目代碼](CodeSystem-cs-tw-ltc-service-item.md) |
...
| | |
| :--- | :--- |
| 服務紀錄申報 | [LTCFeeApplyModel](StructureDefinition-LTCFeeApplyModel.md) |
| 分案狀態、交易結果及各項異動操作 | [LTCFeeAuditStatusModel](StructureDefinition-LTCFeeAuditStatusModel.md) |
| 分案審核明細 | [LTCFeeAuditDetailModel](StructureDefinition-LTCFeeAuditDetailModel.md) |

各 Profile 的「Examples」頁籤可查看填寫範例，也可從[範例總覽](examples.md)查找。
...
| | | |
| :--- | :--- | :--- |
| 服務類別 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-fee-type.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-fee-type.md) |
| 服務項目 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-svc-item.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-svc-item.md) |
| 服務對象 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-svc-people.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-svc-people.md) |
| 服務重點 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-svc-point.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-svc-point.md) |
| 專業服務復能目標達成情形 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-svcc-goal-type.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-svcc-goal-type.md) |
| 社區式服務交通接送服務使用類型 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-bd03-type.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-bd03-type.md) |
| AA10 申報狀態 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-aa10-status.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-aa10-status.md) |
| 服務紀錄補充資訊類別 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-supporting-info.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-supporting-info.md) |
| 核銷狀況 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-case-status.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-case-status.md) |
| API 執行狀況 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-api-status.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-api-status.md) |
| API 功能 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-api-function.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-api-function.md) |
| 核定金額類別 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-adjudication.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-adjudication.md) |
| 清冊文件類別 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-doc-type.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-doc-type.md) |
| 錯誤代碼 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-error-code.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-error-code.md) |
| API 回覆結果代碼 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-rtncode.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-rtncode.md) |
| 縣市代碼 | [代碼定義](CodeSystem-cs-tw-ltc-feeaudit-city.md) | [值集](ValueSet-vs-tw-ltc-feeaudit-city.md) |
...
校驗碼計算、Base64 編碼與 IP 白名單設定，請依原 API 規格書辦理。回覆封套的 `rtncode` 可查閱 [API 回覆結果代碼](ValueSet-vs-tw-ltc-feeaudit-rtncode.md)。
