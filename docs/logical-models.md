...
[居家護理個案基本資料邏輯模型](StructureDefinition-HNBaseDataModel.md)／[居家護理主題與模型清單](home-nursing.md)

**用途**: 描述居家護理照護管理系統 V5.0.16 的資料結構，涵蓋十二支 API 的請求資料、二十四種表單及上傳接收回覆。

**主要組成部分**:

- **個案與收案資料**: 包含個案識別、基本資料、聯絡人、收案日期與負責機構
- **全人評估**: 包含十三類評估表單的題目、答案、評估日期與評估人員
- **照護計畫與評值**: 包含需求摘要、照護目標、措施、執行評值與目標結案
- **照護紀錄與共照**: 包含照護活動、共照內容、人員事件、足部護理及多筆傷口紀錄
- **生命徵象**: 包含體溫、脈搏、呼吸、血壓、血糖、血氧及量測時間
- **上傳與查詢**: 包含表單上傳、接收回覆、日期區間與追蹤碼查詢

**Mapping 目標**: [居家護理逐欄 Mapping](home-nursing-mapping.md)，對應個案、收案療程、問卷回覆、照護計畫、目標、觀察紀錄與處理任務等資源。

**使用場景**: 適用於居家護理機構整合收案、評估、照護執行及追蹤資料，作為來源表單與 FHIR 資源轉換的依據。

##### 在宅急症照護資料集邏輯模型 (HAHCareDataset)

[在宅急症照護資料集邏輯模型](StructureDefinition-HAHCareDataset.md)

**用途**: 描述在宅急症照護的完整資料結構，以 150 個資料元素串聯收案評估、整段療程、單次訪視、臨床照護及結案轉銜。

**主要組成部分**:

- **個案與收案評估**: 包含個案識別、聯絡人、適用計畫、居家環境、照顧者支援及收案建議
- **療程與訪視**: 包含收案期間、整段照護、實地或遠距訪視、照護團隊及工作排程
- **診斷與照護計畫**: 包含疾病與照護問題、預期目標、計畫、服務請求及實際處置
- **量測與檢驗**: 包含生命徵象、檢驗結果、檢體、報告及量測設備
- **用藥與同意**: 包含處方、實際給藥、輸注期間、過敏資訊、照護同意及附件
- **照會與轉銜**: 包含會診回覆、交班內容、療程結束原因、接收機構及後續照護摘要

**Mapping 目標**: [在宅急症資料元素對照表](hah-mapping.md)，對應 Patient、EpisodeOfCare、Encounter、CarePlan、Observation、MedicationAdministration、Composition 等資源。

**使用場景**: 適用於在宅急症照護團隊交換同一次療程的臨床資料，以及結案或轉院時提供完整照護摘要。流程與 Profiles 詳見[在宅急症主題](hah.md)。
...
1. **FHIR Profile**: 定義這些資料如何在 FHIR 資源中具體實現

#### 使用建議

1. **資料準備階段**: 使用邏輯模型了解需要收集的資料欄位
1. **實作階段**: 參考對應的 FHIR Profile 了解如何將資料轉換為 FHIR 資源
1. **驗證階段**: 使用邏輯模型檢查是否遺漏重要資料欄位
