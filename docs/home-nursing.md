...
[逐欄對應表](home-nursing-mapping.md) 列出來源欄位、評估題目、Logical Model 與 QuestionnaireResponse 路徑。表單定義及原生臨床資源均可由 [規範文件](artifacts.md) 下載。
...
| | | | |
| :--- | :--- | :--- | :--- |
| `BaseData` | 收案、個案背景、主要照顧者、共照團隊 | [基本資料](StructureDefinition-HNBaseDataAPIModel.md) | 1–13 |
| `Evaluation` | 同一個案及收案日期下的十三類評估，可各自上傳多個日期 | [全人評估](StructureDefinition-HNEvaluationAPIModel.md) | 14–70 |
| `CaseSummary` | 評估後的需求項目與摘要備註 | [需求摘要](StructureDefinition-HNCaseSummaryAPIModel.md) | 71–74 |
| `CarePlan` | 需求摘要下的目標、措施及評值紀錄 | [照護計畫](StructureDefinition-HNCarePlanAPIModel.md) | 75–80 |
| `CareRecord` | 照護紀錄、資源、服務、事件、生命徵象、多筆傷口與足部護理 | [照護紀錄](StructureDefinition-HNCareRecordAPIModel.md) | 81–121 |
| `CaseDesc` | 共照成員的照護時段與文字紀錄 | [共照紀錄](StructureDefinition-HNCaseDescAPIModel.md) | 122–125 |
| `StaffEgy` | 工作人員緊急事件，不要求個案識別或收案日期 | [人員緊急事件](StructureDefinition-HNStaffEgyAPIModel.md) | 126–129 |
| `CaseClose` | 個案結案日期、人員與原因 | [個案結案](StructureDefinition-HNCaseCloseAPIModel.md) | 130–132 |
| `CarePlanClose` | 指定目標的結案日期及結案人員 | [計畫結案](StructureDefinition-HNCarePlanCloseAPIModel.md) | 133–135 |
| `VitalSign` | 獨立上傳生命徵象 | [生命徵象](StructureDefinition-HNVitalSignAPIModel.md) | 136–138 |
| `GetLog` | 以`start_date`及選填的`end_date`查詢處理結果 | [日期查詢](StructureDefinition-HNGetLogAPIModel.md) | 139 |
| `GetLogByTicket` | 以八位數字`ticket`查詢處理結果 | [追蹤碼查詢](StructureDefinition-HNGetLogByTicketAPIModel.md) | 139 |
...
| | | |
| :--- | :--- | :--- |
| [HNPatient](StructureDefinition-HNPatient.md) | LTCPatient | 沿用識別、中文姓名、地址及緊急聯絡人，新增來源表單關聯 |
| [HNEpisodeOfCare](StructureDefinition-HNEpisodeOfCare.md) | LTCEpisodeOfCareBase | 區分多次收案及結案，新增識別與來源表單 |
| 二十三種個案表單 Response | LTCQuestionnaireResponse | 繼承問卷、個案及填寫時間，新增固定問卷、Slice、型態與選項繫結 |
| [HNStaffEgyResponse](StructureDefinition-HNStaffEgyResponse.md) | FHIR QuestionnaireResponse | 既有表單主體限 LTCPatient；工作人員改以 Practitioner 為主體，建立者為 Organization |
| 二十四張 Questionnaire | LTCQuestionnaire | 定義所有表單欄位、題目、重複結構及可選答案 |
| [HNCarePlan](StructureDefinition-HNCarePlan.md) | LTCCarePlan | 串聯目標、措施、評值及停止資訊 |
| [HNGoal](StructureDefinition-HNGoal.md) | LTCGoal | 保留文字敘述，新增主要目標旗標及來源表單 |
| [HNCommunication](StructureDefinition-HNCommunication.md) | LTCCommunicationServiceA | 共照紀錄、提供者及完整來源表單 |
| [HNVitalSigns](StructureDefinition-HNVitalSigns.md) | FHIR R4 vitalspanel | 以 hasMember 重用既有體溫、心率、呼吸、血壓及血氧 Profile；血糖與量測組使用共同來源表單 |
| 血糖 | 直接重用 PASportObservationGlucose | 沿用 LOINC 2339-0 及 UCUM mg/dL |
| [HNWound](StructureDefinition-HNWound.md) | TW Core Observation-simple | 既有壓傷 Condition 的語意無法涵蓋其他傷口；每個傷口建立一筆 Observation |
| [HNAPITask](StructureDefinition-HNAPITask.md) | LTCTask | 十二種作業、查詢輸入、接收訊息及共照名單更新指示 |
| [HNOperationOutcome](StructureDefinition-HNOperationOutcome.md) | FHIR OperationOutcome | 既有 Outcome 為其他來源系統專用，因此另定義居護訊息 |
...
| | | |
| :--- | :--- | :--- |
| `HealthyHabits` | [健康紀錄](StructureDefinition-HNHealthyHabitsResponse.md) | 菸酒檳榔狀態及年數、過敏、疫苗 |
| `MedicalHistories` | [疾病史](StructureDefinition-HNMedicalHistoriesResponse.md) | 主次診斷、十四類疾病史 |
| `DrugSafeties` | [藥物安全性](StructureDefinition-HNDrugSafetiesResponse.md) | 長期用藥、多重用藥、自行購藥及使用中藥物 |
| `BodyEvaluations` | [身體評估](StructureDefinition-HNBodyEvaluationsResponse.md) | 意識、視聽力、溝通、進食、排泄、皮膚、肌力、行動及呼吸輔助 |
| `PressureInjuries` | [壓力性損傷危險](StructureDefinition-HNPressureInjuriesResponse.md) | 六項危險評估 |
| `FallRisks` | [跌倒危險性](StructureDefinition-HNFallRisksResponse.md) | 八項危險因素及多選說明 |
| `ADLs` | [日常生活功能](StructureDefinition-HNADLsResponse.md) | 十項日常生活活動 |
| `IADLs` | [工具性日常生活活動](StructureDefinition-HNIADLsResponse.md) | 八項工具性活動 |
| `Dementias` | [認知功能](StructureDefinition-HNDementiasResponse.md) | 日期、地點、個人資訊等題目；電話及住址依來源續答規則處理 |
| `GeriatricDepressionScales` | [情緒問題](StructureDefinition-HNGeriatricDepressionScalesResponse.md) | 五項情緒問題 |
| `MNASFs` | [簡易營養](StructureDefinition-HNMNASFsResponse.md) | 營養評估；BMI 無法取得時使用圍度替代題 |
| `PainEvaluations` | [疼痛](StructureDefinition-HNPainEvaluationsResponse.md) | 可言語、無法言語及其他情形，分別保留 Questions 與 QuestionsNot |
| `SOFs` | [衰弱](StructureDefinition-HNSOFsResponse.md) | 體重減輕、下肢功能及活力降低 |
...
[上傳接收回覆模型](StructureDefinition-HNUploadAcknowledgementModel.md) 記錄每支上傳 API 的 `{ "code": 200, "message": "Success." }` 只代表檔案接收成功，系統另以排程匯入。範例 Task 使用 `received` 及 `uploaded`。後續確認匯入成功才設為 completed；若處理失敗則記錄失敗訊息。規範沒有列出 GetLog／GetLogByTicket 的回傳 schema、完整錯誤碼或追蹤碼配發欄位，因此本 IG 不虛構這些內容。
...
| | |
| :--- | :--- |
| 收案 | [Patient](Patient-hn-patient-example.md)、[EpisodeOfCare](EpisodeOfCare-hn-episode-example.md)、[基本資料表單](QuestionnaireResponse-hn-basedata-example.md) |
| 評估 | [身體評估](QuestionnaireResponse-hn-bodyevaluations-example.md)、[ADL](QuestionnaireResponse-hn-adls-example.md)、[疼痛](QuestionnaireResponse-hn-painevaluations-example.md)、[可言語](QuestionnaireResponse-hn-pain-verbal-example.md)、[無法言語](QuestionnaireResponse-hn-pain-nonverbal-example.md) |
| 計畫 | [交易 Bundle](Bundle-hn-careplan-transaction-example.md)、[Goal](Goal-hn-goal-example.md)、[CarePlan](CarePlan-hn-careplan-example.md) |
| 足部護理 | [十七組評估與九十二項操作](QuestionnaireResponse-hn-foot-record-example.md) |
| 多筆傷口 | [完整表單](QuestionnaireResponse-hn-carerecord-wounds-example.md)、[無法分期壓傷](Observation-hn-wound-pressure-example.md)、[失禁性皮膚炎](Observation-hn-wound-iad-example.md)、[其他傷口](Observation-hn-wound-other-example.md) |
| 生命徵象 | [交換 Bundle](Bundle-hn-vital-bundle-example.md)、[量測組](Observation-hn-vital-signs-example.md)、[血糖](Observation-hn-glucose-example.md) |
| 共照與人員事件 | [Communication](Communication-hn-communication-example.md)、[人員事件表單](QuestionnaireResponse-hn-staffegy-example.md) |
| 接收及查詢 | [上傳已接收](Task-hn-upload-task-example.md)、[日期查詢](Task-hn-getlog-task-example.md)、[追蹤碼查詢](Task-hn-ticket-task-example.md) |
...
[在宅急症照護](hah.md)擴充急症收案、實地／遠距訪視、檢驗、給藥及結案轉銜。此子主題重用共同的 LTC 定義；具體資料集與逐欄對照請見[在宅急症 Logical Model](StructureDefinition-HAHCareDataset.md)及[資料對照表](hah-mapping.md)。
